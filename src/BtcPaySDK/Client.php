...
use BtcPaySDK\Exceptions\BillCreationException;
use BtcPaySDK\Exceptions\BillDeliveryException;
use BtcPaySDK\Exceptions\BillQueryException;
use BtcPaySDK\Exceptions\BillUpdateException;
use BtcPaySDK\Exceptions\BitPayException;
use BtcPaySDK\Exceptions\CurrencyQueryException;
use BtcPaySDK\Exceptions\InvoiceCreationException;
use BtcPaySDK\Exceptions\InvoiceQueryException;
use BtcPaySDK\Exceptions\LedgerQueryException;
use BtcPaySDK\Exceptions\PayoutCancellationException;
use BtcPaySDK\Exceptions\PayoutCreationException;
use BtcPaySDK\Exceptions\PayoutQueryException;
use BtcPaySDK\Exceptions\RateQueryException;
use BtcPaySDK\Exceptions\RefundCreationException;
use BtcPaySDK\Exceptions\RefundQueryException;
use BtcPaySDK\Exceptions\SettlementQueryException;
use BtcPaySDK\Exceptions\SubscriptionCreationException;
use BtcPaySDK\Exceptions\SubscriptionQueryException;
use BtcPaySDK\Exceptions\SubscriptionUpdateException;
use BtcPaySDK\Model\Bill\Bill;
use BtcPaySDK\Model\Facade;
use BtcPaySDK\Model\Invoice\Invoice;
use BtcPaySDK\Model\Invoice\Refund;
use BtcPaySDK\Model\Ledger\Ledger;
use BtcPaySDK\Model\Payout\PayoutBatch;
use BtcPaySDK\Model\Rate\Rates;
use BtcPaySDK\Model\Settlement\Settlement;
use BtcPaySDK\Model\Subscription\Subscription;
use BtcPaySDK\Util\JsonMapper\JsonMapper;
use BtcPaySDK\Util\RESTcli\RESTcli;
use Exception;
use http\Url;
...
class Client
{
     ...
     * @param $url              String The URL to your BTCPayServer Instance.
     * @param $privateKeySecret String|null Private Key encryption password.
     * @return Client
     * @throws BitPayException BitPayException class
     */
    public function withData($environment, $privateKey, Tokens $tokens, $url, $privateKeySecret = null)
    {
        try {
            $this->_env = $environment;
            $this->buildConfig($privateKey, $tokens, $url, $privateKeySecret);
            $this->initKeys();
            $this->_url = $url;
            $this->init();
     ...
     * @throws BitPayException BitPayException class
     */
    private function buildConfig($privateKey, $tokens, $url, $privateKeySecret = null)
    {
        try {
    ...
    public function getConfig()
    {
        try {
            ...
            $url = $configData["BitPayConfiguration"]["EnvConfig"][$this->_env]["Url"];
            $this->_url = $url;
    ...
    {
        try {
            $this->_RESTcli = new RESTcli($this->_env, $this->_ecKey, $this->_url);
