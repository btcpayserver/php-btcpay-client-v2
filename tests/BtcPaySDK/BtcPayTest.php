...
namespace BtcPaySDK\Tests;


use BtcPaySDK;
use BtcPaySDK\Model\Bill\BillStatus;
use BtcPaySDK\Model\Currency;
use BtcPaySDK\Model\Invoice\Invoice as Invoice;
use BtcPaySDK\Model\Payout\PayoutStatus;
use PHPUnit\Framework\TestCase;

class BtcPayTest extends TestCase
{
    /**
     * @var BtcPaySDK\Client
     */
    protected $client;
    protected $client1;
    protected $client2;
    ...
    protected function setUp(): void
    {
         ...
         * */
        $this->clientMock = $this->createMock(BtcPaySDK\Client::class);
         ...
         * */
//        $this->client1 = BtcPaySDK\Client::create()->withFile(__DIR__."/../../examples/BitPay.config.json");
//        $this->client2 = BtcPaySDK\Client::create()->withFile(__DIR__."/../../examples/BitPay.config.yml");
...
//        $this->assertNotNull($this->client2);
    ...
    public function testShouldGetInvoiceId()
    {
        ...
        $buyer = new BtcPaySDK\Model\Invoice\Buyer();
    ...
    public function testShouldCreateGetCancelRefundRequest()
    {
        ...
        try {
            ...
            $invoices = $this->client->getInvoices(
                $sevenDaysAgo, $today, BtcPaySDK\Model\Invoice\InvoiceStatus::Complete);
            /**
             * var Invoice
             */
    ...
    public function testShouldCreateBillUSD()
    {
        ...
        $item = new BtcPaySDK\Model\Bill\Item();
        $item->setPrice(30.0);
        $item->setQuantity(9);
        ...
        $bill = new BtcPaySDK\Model\Bill\Bill("1001", Currency::USD, "", $items);
        $basicBill = null;
        try {
    ...
    public function testShouldCreateBillEUR()
    {
        ...
        $bill = new BtcPaySDK\Model\Bill\Bill("1002", Currency::EUR, "", $items);
        $basicBill = null;
        try {
            $basicBill = $this->client->createBill($bill);
        } catch (\Exception $e) {
    ...
    public function testShouldGetBill()
    {
        ...
        $bill = new BtcPaySDK\Model\Bill\Bill("1003", Currency::EUR, "", $items);
        $basicBill = null;
        $retrievedBill = null;
        try {
            $basicBill = $this->client->createBill($bill);
    ...
    public function testShouldUpdateBill()
    {
        ...
        $bill = new BtcPaySDK\Model\Bill\Bill("1004", Currency::EUR, "", $items);
        $basicBill = null;
        $retrievedBill = null;
        $updatedBill = null;
        try {
    ...
    public function testShouldDeliverBill()
    {
        ...
        $bill = new BtcPaySDK\Model\Bill\Bill("1005", Currency::EUR, "", $items);
        $basicBill = null;
        $retrievedBill = null;
        $result = null;
        try {
    ...
    public function testShouldSubmitPayoutBatch()
    {
        ...
        $instructions = [
            new BtcPaySDK\Model\Payout\PayoutInstruction(100.0, "mtHDtQtkEkRRB5mgeWpLhALsSbga3iZV6u"),
            new BtcPaySDK\Model\Payout\PayoutInstruction(200.0, "mvR4Xj7MYT7GJcL93xAQbSZ2p4eHJV5F7A"),
        ];

        $batch = new BtcPaySDK\Model\Payout\PayoutBatch($currency, $effectiveDate, $instructions);
        try {
    ...
    public function testShouldCreateSubscription()
    {
        ...
        $item = new BtcPaySDK\Model\Subscription\Item();
        $item->setPrice(30.0);
        $item->setQuantity(9);
        $item->setDescription("product-a");
        array_push($items, $item);
        ...
        $billData = new BtcPaySDK\Model\Subscription\BillData(
            Currency::USD,
            "",
            $dueDate,
            $items
        ...
        $subscription = new BtcPaySDK\Model\Subscription\Subscription();
